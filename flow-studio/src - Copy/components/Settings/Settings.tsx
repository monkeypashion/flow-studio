...
export const Settings: React.FC = () => {
  ...
  const [loadResults, setLoadResults] = useState<Record<string, { success: boolean; message: string; totalAssets?: number }>>({});
  ...
  const handleLoadAssets = async (tenantId: string) => {
    setLoadingAssets(tenantId);
    setLoadResults((prev) => {
      const next = { ...prev };
      delete next[tenantId];
      return next;
    ...
    const result = await loadAssetsFromCredential(tenantId);
    setLoadResults((prev) => ({ ...prev, [tenantId]: result }));
            ...
            <div className="space-y-3">
              {tenants.map((tenant) => (
                ...
                >
                  ...
                  {/* Load Assets Result */}
                  {loadResults[tenant.id] && (
                    <div
                      className={`mb-3 px-2 py-1.5 rounded text-xs ${
                        loadResults[tenant.id].success
                          ? 'bg-green-900/30 text-green-300 border border-green-700'
                          : 'bg-red-900/30 text-red-300 border border-red-700'
                      }`}
                    >
                      {loadResults[tenant.id].message}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleLoadAssets(tenant.id)}
                        disabled={loadingAssets === tenant.id}
                        className="flex-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {loadingAssets === tenant.id ? 'Loading...' : 'Load Assets'}
                      </button>
                    </div>
                    <div className="flex gap-2">
                      {!tenant.isDefault && (
