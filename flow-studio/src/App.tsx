...
function App() {
  ...
  useEffect(() => {
    ...
    // Migration: If no data jobs but groups exist, migrate to default job
    setTimeout(() => {
      const state = useAppStore.getState();
      if (state.dataJobs.length === 0 && state.groups.length > 0) {
        console.log('Migrating existing groups to default job...');
        const jobId = state.addDataJob('Default Job');
        state.addAssetsToJob(jobId, state.groups);
