...
export const Clip: React.FC<ClipProps> = ({
  ...
  // Helper function to get all snap points from other clips
  const getSnapPoints = (excludeClipIds: string[] = []) => {
    const snapPoints: number[] = [];
    const allTracks = getAllTracks();
    const SNAP_THRESHOLD = 0.2; // seconds - adjust for sensitivity
  ...
  const createGhostClipsForSelection = useCallback((targetTrackIndex: number, startTrackIndex: number, deltaTime: number, isIncompatible: boolean) => {
    ...
    return selectedClips.map((selectedClip) => {
      ...
      if (ghostTrackElement && timelineContainer) {
        ...
        ghostTop = trackRect.top - timelineRect.top + timeline.scrollY + 4;
      }
  ...
  }, [getSelectedClips, getAllTracks, getTrack, timeline.scrollY, zoom]);
  ...
  const handleMouseDown = (e: React.MouseEvent, type: 'move' | 'resize-left' | 'resize-right') => {
    ...
    if (trackElement && timelineContainer) {
      ...
      // Get the scrollable container too
      const scrollContainer = timelineContainer.parentElement as HTMLElement;
      const scrollRect = scrollContainer?.getBoundingClientRect();

      // Calculate positions
      initialGhostTop = trackRect.top - timelineRect.top + timeline.scrollY + 4; // +4 for clip padding

      // Debug: Verify ghost positioning
      const clipRelativeToTrack = clipRect.left - trackRect.left;
      const clipRelativeToTimeline = clipRect.left - timelineRect.left;
      console.log('Ghost positioning:', {
        clipLeft: left,
        ghostLeft: initialGhostLeft,
        clipInDOM: clipRelativeToTimeline,
        ghostShouldBe: clipRelativeToTimeline,
        matches: Math.abs(initialGhostLeft - clipRelativeToTimeline) < 2 ? '✓' : '✗'
      });
    }
          ...
          if (shouldCopy !== dragInfo.current.isCopyMode) {
            ...
            if (!shouldCopy) {
              ...
              if (dragInfo.current.isMultiSelect) {
                ...
                // Find the target track element using data attribute
                ...
                if (targetTrackElement && timelineContainer) {
                  ...
                  const timelineRect = timelineContainer.getBoundingClientRect();
                  targetTop = trackRect.top - timelineRect.top + timeline.scrollY + 4;
                }
